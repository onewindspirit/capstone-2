...
class ImagePipeline(object):
    ...
    def filter_img_lst(self,sub_dir='all',min_res=32,only_squares=True):
        '''
        returns a filtered image list based on resolution and shape
        Inputs:
        ...
        '''
        sub_dir_ind = self.img_lst2[self.label_map[sub_dir]]
        new_img_lst = []
        if only_squares == True:
            for img in sub_dir_ind:
            ...
            for img in sub_dir_ind:
                if img.shape[0]>=min_res & img.shape[1]>=min_res:
                    new_img_lst.append(img)
        self.img_lst2 = np.asarray(new_img_lst).reshape(1,len(new_img_lst)).tolist()
